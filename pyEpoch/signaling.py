...
def mean_module_expression(adata,module_dict, treatment, result_name):
    adata_sub = adata[adata.uns['cells']['treatment']== treatment, :]
    # mean_dict={'cell':adata_sub.obs_names}
    res=pd.DataFrame(index=module_dict.keys(),columns=adata_sub.obs_names)
    
    for module in module_dict:
        genes = adata_sub.var_names.intersection(module_dict[module])
        subset = adata_sub[:, genes]
        if(len(genes)==1):
            res.loc[module] = subset
        else:
            res.loc[module] = subset.X.mean(axis = 1)
...
#     print("Mean module expression stored in adata.uns['"+result_name+"']")
#     return(adata)
